...
# 2) Kill any stray mpv processes
# 3) Fetch camera list if needed
# 4) Run layout chooser (continue on non-zero exit)
# 5) Validate config
# 6) Detect resolution & compute grid math
# 7) Per-tile: pkill old + launch exactly one mpv
# 8) Start monitor_streams.py in background, then wait
# -----------------------------------------------------------------------------
...
# 3) Aggressively kill any mpv
echo "[INFO] Killing all existing mpv (SIGKILL)"
pkill -9 -f mpv || true
sleep 1
...
# 6) Layout chooser (ignore non-zero exit)
...
# 10) HWDEC hint: Pi5 only; others auto-select
...
# 11) Read tiles into Bash array
mapfile -t TILES < <($JQ_BIN -c '.tiles[]' "$CONFIG")

# 12) For each tile: kill old then launch one mpv
for tile in "${TILES[@]}"; do
  ...
  TITLE="tile_${R}_${C}"
  echo "[INFO] Killing existing ${TITLE}"
  # `--` stops pkill from parsing the next argument as an option
  pkill -9 -f -- "--title=${TITLE}" || true

  # Convert secure RTSP to plain RTSP
...
# 13) Start monitor_streams.py and wait
